...
export default function PantrySearchPage() {
  ...
  const [ingredients, setIngredients] = useState<string[]>(['Pollo', 'Tomates']);
    ...
    .map((recipe) => {
      const matchCount = recipe.ingredients.filter((recipeIngredient) =>
        ingredients.some((userIngredient) =>
          recipeIngredient.item.toLowerCase().includes(userIngredient.toLowerCase())
        )
      ).length;
            ...
            <CardContent>
                ...
                <div className="flex flex-wrap gap-2">
                    {ingredients.map((ingredient) => (
                        <Badge key={ingredient}>{ingredient} <Button variant="ghost" size="icon" className="h-4 w-4 ml-1" onClick={() => handleRemoveIngredient(ingredient)}><X className="h-3 w-3"/></Button></Badge>
                    ))}
                </div>
            </CardContent>
        </Card>
